...
description: '실무에서 Validation Check는 필수이다. 실제로 개발하면서 수없는 비지니스 로직이 따르는데, 거기엔 항상 ‘그’가 존재한다'
...
실무에서 Validation Check는 필수이다. 실제로 개발하면서 수없는 비지니스 로직이 따르는데, 거기엔 항상 ‘그’가 존재한다. 바로 ‘Validation’ 프론트 개발자라면 그냥 지나쳐선 절대로 안되는 필수로직이기 때문에 클린코드 부분에서 많은 생각을 할 수 밖에없다.
