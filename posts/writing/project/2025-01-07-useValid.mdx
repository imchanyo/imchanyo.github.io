...
```typescript
  ...
  const saveValidation = () => {
    ...
    const { brndId, conrBrndDspyYn, conrBrndDspySeq, conrBrndDspyBegnDt, conrBrndDspyEndDt } = values.value
    // 기본정보 필수값 체크
    if (
...
export const useValid = () => {
  ...
  const runValidation = async <T>({ params , schema }: RunValidationModel<T>): Promise<void> => {
    try {
...
```ts
 const processCommonSchema = <T>(params: T, commonSchema: RequireParams): Record<string, any> => {
...
const createCommonSchema = (commonSchema: RequireParams = {}): Record<string, any> => {
    return Object.entries(commonSchema).reduce(
      (acc, [key, value]) => {
        ...
        fieldEntries.forEach(([field, fieldValue]) => {
          ...
          const schemaType = type === 'string' ? string().nonNullable().trim() : typeObj[type] // type이 없거나 잘못된 경우 기본값으로 string 사용
