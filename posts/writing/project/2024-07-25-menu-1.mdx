...
이 메뉴 리스트 데이터는 세션 스토리지에 저장됩니다. 세션 스토리지에 저장하는 이유는 다음과 같습니다
메뉴 리스트는 개발자와의 사전 협의 없이 추가할 수 없기 때문에 페이지 이동 시마다 API 호출이 불필요합니다.
...
예를 들어, 3단계에 있는 서비스 바로가기에서 데이터를 찾으려면, 키값이 `MAB025`인 노드를 트리 구조에서 찾아야 하며, 
이 경우 `O(n)`의 시간 복잡도가 발생할 수 있습니다. 이처럼 키값을 이용해 특정 노드만 찾는 로직은 여러 곳에서 사용됩니다.

예를 들어, 3단계에 있는 서비스 바로가기에서 삭제를 수행한다고 가정해 보겠습니다.
...
<img src="/img/project/menu-6.png" width="100%" />
